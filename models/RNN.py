...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE  
      L = tf.get_variable(name = "L", shape = (len(self.vocab), self.config.embed_size))
      inputs = tf.nn.embedding_lookup(L, self.input_placeholder)
      inputs = tf.split(1, self.config.num_steps, inputs)
      inputs = [tf.squeeze(inputAtSingleTimeStep,[1]) for inputAtSingleTimeStep in inputs]
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    loss = sequence_loss([output], [tf.reshape(self.labels_placeholder, shape = (-1,))], 
  ...
  def add_model(self, inputs):
	...
	I = tf.get_variable("I", shape = (self.config.embed_size, self.config.hidden_size))
	    ...
	    i = tf.nn.dropout(i, self.dropout_placeholder)
            if count == 0:
		hidden = self.initial_state
	    else: 
		hidden = current_output
            current_output = tf.sigmoid(tf.matmul(hidden, H) + tf.matmul(i, I) + b1)
            current_output = tf.nn.dropout(current_output, self.dropout_placeholder)
            rnn_outputs.append(current_output)
    self.final_state = current_output
    ### END YOUR CODE
    return rnn_outputs
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    token = tokens[-1] #Last element of this list
	...
	[model.final_state, model.predictions[-1]], feed_dict = feed_dict_generate)
    ### END YOUR CODE
