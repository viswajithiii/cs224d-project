...
class OpinExtractSeq2SeqModel(object):
  ...
  def __init__(self, vocab, buckets, size,
    ...
    Args:
      vocab: the instance of Vocab which contains the vocabulary.
    ...
    if num_samples > 0 and num_samples < len(self.vocab):
      with tf.device("/cpu:0"):
        w = tf.get_variable("proj_w", [size, len(self.vocab)])
        w_t = tf.transpose(w)
        b = tf.get_variable("proj_b", [len(self.vocab)])
    ...
    if self.verbose:
        print (time.ctime(), "Defined loss function.")
    ...
    if self.verbose:
        print (time.ctime(), "Added placeholders.")
    ...
    if self.verbose:
        print (time.ctime(), "Defined seq2seq model.")
