...
# batch_size, buckets, steps per val eval
...
def train():
  ...
  with tf.Session() as sess:
    ...
    dev_set = read_data('data/val_tokens.txt', vocab)
    train_set = read_data('data/train_tokens.txt', vocab)
    train_bucket_sizes = [len(train_set[b]) for b in xrange(len(_buckets))]
    print ('Train bucket sizes:', train_bucket_sizes)
    print ('Val bucket sizes:', [len(dev_set[b]) for b in range(len(_buckets))])
    ...
    while True:
      ...
      print ('Starting batch at', time.ctime())
      encoder_inputs, decoder_inputs, target_weights = model.get_batch(
          train_set, bucket_id)
      _, step_loss, _, summ = model.step(sess, encoder_inputs, decoder_inputs,
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        if current_step % steps_per_val_eval == 0:
          ...
          for bucket_id in xrange(len(_buckets)):
            ...
            encoder_inputs, decoder_inputs, target_weights = model.get_batch(
                dev_set, bucket_id)
            _, eval_loss, outputs, _ = model.step(sess, encoder_inputs,
                                                  decoder_inputs,
                                         target_weights, bucket_id, True)
            arg_max_outputs = [np.argmax(o, 1) for o in outputs]
            with open('decoder_outputs.txt', 'a') as out_f:
...
def decode():
  ...
  with tf.Session() as sess:
    ...
    model = create_model(sess, True, vocab, embedding_matrix=em_mat)
    ...
    while sentence:
      ...
      token_ids = [vocab.encode(x) for x in nltk.word_tokenize(sentence)]
      ...
      print(" ".join([vocab.decode(output) for output in outputs]))
      print("> ", end="")
      sys.stdout.flush()
      sentence = sys.stdin.readline()
...
def self_test():
  ...
  with tf.Session() as sess:
    ...
    model = OpinExtractSeq2SeqModel(10, 10, [(3, 3), (6, 6)], 32, 2,
    ...
    for _ in xrange(5):  # Train the fake model for 5 steps.
      ...
      encoder_inputs, decoder_inputs, target_weights = model.get_batch(
          data_set, bucket_id)
      print(encoder_inputs, decoder_inputs, target_weights)
