...
test_claims = [claim for claim in iDebateDump if claim['_debate_id'] in test_debates]
train_claims = [claim for claim in iDebateDump if claim['_debate_id'] in train_debates]
val_claims = [claim for claim in iDebateDump if claim['_debate_id'] in val_debates]
assert len(test_claims) + len(train_claims) + len(val_claims) == len(iDebateDump) 
