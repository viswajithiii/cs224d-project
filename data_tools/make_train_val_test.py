...
def write_data_to_file(read_f, write_f):
...
if __name__ == "__main__":
	...
	test_f = open('test.txt', 'a')
        dev_f = open('dev.txt', 'a')
	for fname in file_list:
	    full_fname = os.path.join(curr_dir, fname)
	    f = open(full_fname, 'r')
	    if fname in argentina_filenames:
		choice = random.choice('td')
		if choice == 't':
		    write_data_to_file(f, test_f)
		else:
		    write_data_to_file(f, dev_f)
	    else:
		choice = random.choice('r'*170 + 't'*15 + 'd'*15)
		if choice == 'r':
		    write_data_to_file(f, train_f)
		elif choice == 't':
		    write_data_to_file(f, test_f)
		else:
		    write_data_to_file(f, dev_f)
